...
async def get_recommendation(
        ...
        if item["type"] == "movie":
            ...
            for m in liked_movies:
                if str(m["movieId"]) == str(item["id"]):
                    ...
                    break
            if not found:
                 liked_movies.append({"movieId": int(item["id"]), "timestamp": item["timestamp"]})
            ...
            for s in liked_songs:
                if str(s["spotify_id"]) == str(item["id"]):
                    ...
                    break
            if not found:
                liked_songs.append({"spotify_id": item["id"], "timestamp": item["timestamp"]})
    ...
    print(f"[DEBUG] User {body.user_id}: {len(liked_movies)} liked movies, {len(liked_songs)} liked songs")
    ...
    if song_recommender and liked_songs:
        ...
        print(f"[DEBUG] Skipping songs. Recommender: {song_recommender is not None}, Liked Songs: {len(liked_songs)}")
    ...
    nostalgic_candidates = [c for c in candidates if c.get("nostalgia_score", 0) >= MIN_NOSTALGIA_SCORE]
    
    if nostalgic_candidates:
        # Log distribution
        scores = [c["nostalgia_score"] for c in nostalgic_candidates]
        print(f"[NOSTALGIA] Filtered {len(candidates)} -> {len(nostalgic_candidates)} nostalgic candidates (score >= {MIN_NOSTALGIA_SCORE})")
        print(f"[NOSTALGIA] Score range: {min(scores):.2f} - {max(scores):.2f}, avg: {sum(scores)/len(scores):.2f}")
        ...
        print(f"[NOSTALGIA] Warning: No high nostalgia-score candidates. Using {len(candidates)} candidates (all 10+ years old).")
    ...
    if bandit:
        try:
            ...
            candidate_arms = [bandit.global_model._get_arm_from_candidate(c) for c in candidates]
            ...
            selected_arm = bandit.global_model._get_arm_from_candidate(candidates[selected_idx])
            arm_candidates = [
            ...
            if len(arm_candidates) > 1:
                ...
                arm_candidates.sort(key=lambda x: x[1].get("similarity_score", 0), reverse=True)
                top_n = arm_candidates[:5]
                
                import random
                best_idx, selected = random.choice(top_n)
                print(f"[DEBUG] Within-arm selection: picked random from top {len(top_n)} of {len(arm_candidates)} candidates in '{selected_arm}'")
            else:
                selected = candidates[selected_idx]
            
            print(f"[DEBUG] Selected: {selected.get('title') or selected.get('name')} | Arm: {selected_arm} | Nostalgia: {selected.get('nostalgia_score', 0):.2f}")
        except Exception as e:
    ...
    if selected["type"] == "movie":
        content = RecommendedContent(
            ...
            genres=selected.get("genres", "").split("|") if selected.get("genres") else None,
            artists=None,
            name=None,
            genre=None,
        )
        ...
        content = RecommendedContent(
            ...
            artists=selected.get("artists", "").split(", ") if isinstance(selected.get("artists"), str) else selected.get("artists"),
            year=selected.get("year"),
