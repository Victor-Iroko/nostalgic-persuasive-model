...
def evaluate_nostalgia_metrics(
    ...
    for i, row in enumerate(test_songs):
        ...
        if query_artists:
            try:
                query_artist_set = set(query_artists) if isinstance(query_artists, list) else set()
            ...
            for r in recommendations:
                ...
                if rec_artists:
                    try:
                        rec_artist_set = set(rec_artists) if isinstance(rec_artists, list) else set()
        ...
        if query_duration:
            duration_matches = sum(
                1 for r in recommendations
                if r[2] is not None and abs(r[2] - query_duration) <= DURATION_TOLERANCE_MS
            )
    ...
    metrics: dict[str, float] = {
        ...
        "artist_scene_continuity": float(np.mean(artist_continuities)) if artist_continuities else 0.0,
        "mood_consistency": float(np.mean(mood_consistencies)) if mood_consistencies else 0.0,
        "duration_familiarity": float(np.mean(duration_familiarities)) if duration_familiarities else 0.0,
        "n_queries": n_queries,
...
def print_results(metrics: dict[str, float], stats: dict[str, int]) -> None:
    ...
    print(f"   Queries: {metrics['n_queries']}, Recommendations: {metrics['n_recommendations']}")
    ...
    print(f"{era_status} {'Era Recall (±' + str(ERA_WINDOW_YEARS) + ' years)':<38} {era_pct:>14.1f}%")
    
    # Popularity Drift
    pop_drift = metrics['popularity_drift_mean']
    pop_status = "✅" if -15 <= pop_drift <= 0 else "✓" if -30 <= pop_drift <= 10 else "⚠"
    ...
    print(f"{mood_status} {'Mood Consistency (±' + str(MOOD_TOLERANCE) + ')':<38} {mood_pct:>14.1f}%")
    ...
    if pop_drift < 0:
        print(f"   • Popularity: Recommends slightly less popular songs (good for discovery)")
    elif pop_drift > 10:
