...
def compute_movie_ratings(ratings_path: Path) -> pd.DataFrame:
    ...
    for chunk in pd.read_csv(ratings_path, chunksize=1_000_000):
        ...
        grouped = chunk.groupby('movieId').agg(
    ...
    final = combined.groupby('movieId').agg(
    ...
    print(f"Rating count range: {final['ratingCount'].min()} - {final['ratingCount'].max()}")
    print(f"Average rating range: {final['avgRating'].min():.2f} - {final['avgRating'].max():.2f}")
...
def update_database(movie_stats: pd.DataFrame) -> tuple[int, int]:
    ...
    try:
        ...
        for i, row in movie_stats.iterrows():
            ...
            if (i + 1) % 10000 == 0:
                print(f"  Processed {i + 1} movies ({updated} updated, {not_found} not found)")
                conn.commit()
...
def main() -> None:
    ...
    ratings_path = Path(__file__).parent.parent / "dataset" / "ml-32m" / "ratings.csv"
