...
def load_data(sample_n: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not MOVIES_FILE.exists():
        raise FileNotFoundError(f"Movies file not found at {MOVIES_FILE}. Please run preprocess_movies.py first!")
    ...
    print(f"  Filtered to {len(ratings):,} positive ratings (>= {MIN_RATING}) - removed {original_count - len(ratings):,}")
...
def preprocess_data(
    ...
    movie_features_generator = (
        (row.movieId, get_movie_features(row))
...
def main() -> None:
    ...
    try:
        ...
        save_artifacts(model, dataset, item_features, train_interactions, test_interactions)
