...
def print_results(metrics: dict[str, float], k: int = 10) -> None:
    ...
    print(f"{'Precision@' + str(k):<30} {metrics[f'train_precision@{k}']:<12.4f} {metrics[f'test_precision@{k}']:<12.4f}")
    print(f"{'Recall@' + str(k):<30} {metrics[f'train_recall@{k}']:<12.4f} {metrics[f'test_recall@{k}']:<12.4f}")
    ...
    if test_prec >= 0.10:
        print(f"  ✅ Precision@{k}: Strong ({test_prec:.1%} of recommendations are relevant)")
    elif test_prec >= 0.05:
        print(f"  ✓ Precision@{k}: Good ({test_prec:.1%} of recommendations are relevant)")
    else:
...
def main() -> None:
    ...
    try:
        ...
        model, item_features, train_interactions, test_interactions, config = load_artifacts()
